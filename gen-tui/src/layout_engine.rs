    ...
    S: GraphSource<G>,
{
    ...
    /// [`Self::window_for`]/[`Self::activate_batch_containing`], which both also navigate
    /// there. For a caller that needs to merge in data its own way rather than through the
    /// crawl's usual [`crate::crawl::GraphSource::expand_frontier`] path - e.g. gen-python's
    /// `show_path`, which must be able to project a stored path onto the graph regardless of
    /// whether the active source is filtering some of that path's own edges out (see
    /// `SqlGraphSource::new_pruned`). A caller's designated "current path" is not guaranteed to
    /// be made up of exactly the edges a display-oriented pruning policy would keep.
