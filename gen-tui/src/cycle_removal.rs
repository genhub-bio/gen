    ...
    G::NodeId: Copy + Eq + Hash + Ord,
{
    ...
    let mut candidates: BTreeSet<(i32, Reverse<usize>)> = (0..bound)
        .filter(|&index| active[index])
        .map(|index| (out_deg[index] - in_deg[index], Reverse(index)))
        .collect();
    ...
    let remove_node = |vi: usize,
                       ...
                       candidates: &mut BTreeSet<(i32, Reverse<usize>)>| {
        if !active[vi] {
            return;
        }
        candidates.remove(&(out_deg[vi] - in_deg[vi], Reverse(vi)));
        active[vi] = false;
        *active_count -= 1;

        for &w in &outs[vi] {
            if active[w] {
                candidates.remove(&(out_deg[w] - in_deg[w], Reverse(w)));
                in_deg[w] -= 1;
                candidates.insert((out_deg[w] - in_deg[w], Reverse(w)));
                if in_deg[w] == 0 {
                    sources.push_back(w);
                }
            }
        ...
        for &u in &ins[vi] {
            if active[u] {
                candidates.remove(&(out_deg[u] - in_deg[u], Reverse(u)));
                out_deg[u] -= 1;
                candidates.insert((out_deg[u] - in_deg[u], Reverse(u)));
                if out_deg[u] == 0 {
                    sinks.push_back(u);
                }
            }
    ...
    while active_count > 0 {
        ...
        if let Some((_, Reverse(best))) = candidates.pop_last() {
            if !active[best] {
                continue;
            }
            prefix.push(best);
