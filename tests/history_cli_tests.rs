...
use r#gen::{
    core::{HashId, Workspace},
    diff::{
        graph::{DiffChangeKind, DiffPresence},
        operations::collect_operation_diff,
    },
    get_connection,
    patch::load_patches,
};
use gen_models::{
    assets::tables::AssetRef,
...
fn asset_refs(repo_root: &Path) -> Vec<AssetRef> {
    ...
    AssetRef::query(
        &connection,
        "SELECT * FROM gen_asset_refs ORDER BY created_on, role, name",
        [],
    )
}
...
fn test_view_diff_reports_vcf_added_and_removed_nodes_from_deltas() {
    ...
    let diff = collect_operation_diff(&graph_conn, Some(import_hash), update_hash)
        .expect("should collect operation diff");
    let unknown_diff = diff
        .diff_graph
        .iter()
        .find(|diff| {
            diff.block_group.as_ref().is_some_and(|block_group| {
    ...
    let unknown_block_group = unknown_diff
        .block_group
        .as_ref()
        .expect("should include unknown block group metadata");
    let g1_diff = diff
        .diff_graph
        .iter()
        .find(|diff| {
    ...
    let diff_node_ids = unknown_diff
        ...
        .into_iter()
        .collect::<Vec<_>>();
    let sequences_by_node_id = Node::get_sequences_by_node_ids(&graph_conn, &diff_node_ids, None);
...
fn test_patch_apply_round_trips_from_repo_into_fresh_repo() {
    ...
    assert_success(
        &run_gen(
            source_repo.path(),
            &["patch-create", "--name", "round-trip", "HEAD~1..HEAD"],
        ),
...
fn test_patch_apply_preserves_sample_lineage_for_vcf_updates() {
    ...
    assert_eq!(
        SampleLineage::get_parents(&graph_conn, "unknown"),
...
#[test]
fn test_pull_from_file_remote_bootstraps_history_and_asset_refs() {
    let remote_repo_dir = tempdir().expect("should create temp remote repo directory");
    let local_repo_dir = tempdir().expect("should create temp local repo directory");
    let fixtures_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures");
    let fasta_path = fixtures_dir.join("simple.fa");
    ...
    assert_success(
        &run_gen(local_repo_dir.path(), &["pull"]),
        "pull should succeed",
    );

    let pulled_history = operations_stdout(local_repo_dir.path());
    assert!(
        pulled_history.contains("m123:"),
        "pulled history should include the fasta import commit: {pulled_history}"
    );
    assert!(
        pulled_history.contains("add-gff"),
        "pulled history should include the add-file commit: {pulled_history}"
    );

    let pulled_samples = run_gen(local_repo_dir.path(), &["list-samples"]);
    assert_success(&pulled_samples, "list-samples after pull should succeed");
    assert!(
        String::from_utf8_lossy(&pulled_samples.stdout).contains("test-sample"),
        "pull should bootstrap the graph database with the remote sample state"
    );

    let refs = asset_refs(local_repo_dir.path());
    assert!(
        refs.iter()
            .any(|asset_ref| asset_ref.name.as_deref() == Some("simple.gff")),
        "pulled graph db should retain the asset reference rows: {refs:?}"
    );
    assert_remote_asset_is_not_materialized(local_repo_dir.path());
}
