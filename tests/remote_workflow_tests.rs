...
mod remote_workflow_integration_tests {
    ...
    fn test_remote_deletion_with_branch_associations() {
        ...
        RemoteBranch::set_remote_validated(op_conn, "main", Some("origin")).unwrap();
        RemoteBranch::set_remote_validated(op_conn, "feature", Some("origin")).unwrap();
        RemoteBranch::set_remote_validated(op_conn, "develop", Some("upstream")).unwrap();

        // Verify initial associations
        assert_eq!(
            RemoteBranch::get_remote(op_conn, "main"),
            Some("origin".to_string())
        );
        assert_eq!(
            RemoteBranch::get_remote(op_conn, "feature"),
            Some("origin".to_string())
        );
        assert_eq!(
            RemoteBranch::get_remote(op_conn, "develop"),
        ...
        assert_eq!(RemoteBranch::get_remote(op_conn, "main"), None);
        assert_eq!(RemoteBranch::get_remote(op_conn, "feature"), None);
    ...
    fn test_remote_workflow_error_scenarios() {
        ...
        let result = RemoteBranch::set_remote_validated(op_conn, "main", Some("nonexistent"));
        assert!(
            result.is_err(),
