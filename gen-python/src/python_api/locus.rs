...
pub(crate) trait GraphLocusExt {
    /// Total number of bases covered by this locus.
    ...
    /// and `sequence_end` are the node coordinates of the covered bases, so the same
    /// bases produce the same canonical locus no matter how later edits split their
    /// blocks. Consecutive slices of one node that abut on the same strand are merged;
    /// zero-length slices are kept as-is because they address a point between bases.
    fn canonical(&self) -> GraphLocus;

    /// The same bases read from the opposite strand.
    fn reverse_complement(&self) -> GraphLocus;

    /// Sub-locus covering bases `start..end`, counted in reading order.
    ///
    /// Offsets run along the slices in order; within a reverse-strand slice they run
    /// from the slice's end, matching how `GraphLocus::sequence` reads it. `start == end`
    /// yields a zero-length locus addressing the point before base `start`. Returns
    /// `None` when the range is inverted or extends past the end of the locus.
...
impl GraphLocusExt for GraphLocus {
    ...
    fn canonical(&self) -> GraphLocus {
        ...
        for slice in &self.slices {
            ...
            {
                if previous.block.sequence_end == node_start {
    ...
    fn slice(&self, start: usize, end: usize) -> Option<GraphLocus> {
        ...
        for slice in &self.slices {
            ...
            let is_point_here = start == end && start >= offset && start <= offset + length;
            if overlap_start < overlap_end || is_point_here {
...
mod tests {
    ...
    #[test]
    fn test_canonical_keeps_zero_length_points() {
        let point = GraphLocus {
            slices: vec![slice("a", (10, 20), (3, 3), Strand::Forward)],
        };
        assert_eq!(
            point.canonical().slices,
            vec![slice("a", (13, 13), (0, 0), Strand::Forward)],
            "a point should stay a zero-length slice at its node coordinate"
        );
    }
    ...
    fn test_slice_counts_offsets_in_reading_order() {
        ...
        assert_eq!(
            locus.slice(8, 8).expect("should slice a point").slices,
            vec![slice("a", (0, 10), (10, 10), Strand::Forward)],
            "a point on a junction should resolve to the first block's end"
        );
        assert!(
    ...
    fn test_slice_of_reverse_strand_reads_from_slice_end() {
        ...
        assert_eq!(
            ...
            "the first reverse-strand bases should come from the slice end"
        );
    }
}
