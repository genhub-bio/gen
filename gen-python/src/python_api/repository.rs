...
impl PyRepository {
    ...
    #[pyo3(signature = (filename, name=None, genotype=None, sample=None, parent_sample=None, in_place=false))]
    fn update_with_vcf(
        &self,
        filename: String,
        name: Option<String>,
    ...
    #[pyo3(signature = (k=16, bgs=None))]
    fn build_index(&self, k: usize, bgs: Option<Vec<PyBlockGroup>>) -> PyResult<()> {
        let conn = self.context.graph().conn();
        let index_dir = self
            .context
            .workspace()
    ...
    /// Search `block_group`'s graph for exact occurrences of `query`.
    ///
    ...
    fn search(&self, block_group: &PyBlockGroup, query: &str) -> PyResult<Vec<PyGraphLocus>> {
        let conn = self.context.graph().conn();
        let graph = BlockGroup::get_graph(conn, &block_group.id);
        let matcher = GenGraphMatcher::new(conn, graph);
        // DNA sequences are stored uppercase; normalise the query so that
        // a caller passing "ctagctcagt" finds the same matches as "CTAGCTCAGT".
        ...
        let index = index_path
            ...
            .and_then(|bytes| postcard::from_bytes::<SeedIndex>(&bytes).ok());
        ...
        Ok(matches.into_iter().map(PyGraphLocus::from_locus).collect())
    }
...
mod python_tests {
    ...
    fn test_search_finds_exact_match() {
        ...
        Python::with_gil(|py| {
            ...
            let hits = py_repo.borrow(py).search(bg, "ACGT").unwrap();
    ...
    fn test_search_no_match() {
        ...
        Python::with_gil(|py| {
            ...
            let block_groups = py_repo.borrow(py).get_block_groups().unwrap();
            let bg = &block_groups[0];
            let hits = py_repo.borrow(py).search(bg, "NNNN").unwrap();
            assert!(hits.is_empty(), "Expected no matches for 'NNNN'");
        });
    }
    ...
    fn test_build_index_creates_file() {
        ...
        Python::with_gil(|py| {
            ...
            py_repo.borrow(py).build_index(4, None).unwrap();
