...
//! the same [`super::HttpRequest`]/[`super::HttpResponse`] shape fronts both the native and
//! `emscripten` transports.
...
pub fn request(http_request: HttpRequest<'_>) -> Result<HttpResponse, BrowserHttpError> {
    ...
    let mut builder = HTTP_CLIENT.request(method, http_request.url);
