...
fn path_from_segments(sample_name: &str, path: &Path, segments: &[Segment]) -> GFAPath {
    let path_name = format!("{sample_name}.{}", path.name);
    GFAPath {
...
mod tests {
    ...
    fn test_gfa_diff() {
        ...
        let new_child_block_group = Collection::get_block_groups(conn, "test collection 2", None)
            .pop()
            .unwrap();
        let all_child_sequences = gen_models::models::get_all_sequences_with_pruning(
            conn,
    ...
    fn test_gfa_diff_against_nothing() {
        ...
        let new_block_group = Collection::get_block_groups(conn, "test collection 2", None)
            ...
            .unwrap();
        let all_sequences =
            gen_models::models::get_all_sequences_with_pruning(conn, &new_block_group.id, false)
                .unwrap();
