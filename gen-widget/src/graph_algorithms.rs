...
mod articulation_points_tests {
    ...
    #[test]
    fn viewport_visual_regression_extended_complex_dag_with_articulation_points() {
        use crate::{
            geometry::WorldPos,
            ...
            testing::{
                ...
                mocks::{FixedNodeSizer, TestRenderers},
            },
        };

        // Create only the domain graph - let the system do the partitioning
        let domain_graph = TestGraphs::domain_complex_dag();

        let articulation_points = find_articulation_points(&domain_graph);
        println!(
            "Extended Complex DAG articulation points: {:?}",
            articulation_points
        );

        // Create viewport snapshot using GraphController (proper partitioning)
        let mut terminal = create_test_terminal(80, 25);
        let mut renderer = TestRenderers::debug();

        // Create GraphController with domain graph
        let node_sizer = FixedNodeSizer {
            width: 5,
            height: 3,
        };
        // Use smaller partitions to force multi-partition layouts for testing
        let mut config = GraphConfig::default();
        config.partition.layer_count = 3; // Very small partitions to force multiple partitions
        let mut controller = GraphController::new_with_config(&domain_graph, node_sizer, config);

        let test_viewport = ratatui::layout::Rect::new(0, 0, 80, 25);
        ...
        insta::assert_snapshot!(
            "viewport_extended_complex_dag_with_articulation_points",
            snapshot
        );
    }

    #[test]
    fn viewport_visual_regression_extended_diamond_with_articulation_points() {
        use crate::{
            geometry::WorldPos,
            graph_controller::{GraphConfig, GraphController, WorldBuffer},
            layout::VisualDetail,
        ...
        // Create extended diamond graph
        let domain_graph = TestGraphs::domain_extended_diamond();

        let articulation_points = find_articulation_points(&domain_graph);
        println!(
            "Extended Diamond articulation points: {:?}",
            articulation_points
        );
        ...
        let mut config = GraphConfig::default();
        config.partition.layer_count = 3; // Force multiple partitions
        let mut controller = GraphController::new_with_config(&domain_graph, node_sizer, config);
        ...
        insta::assert_snapshot!(
            "viewport_extended_diamond_with_articulation_points",
            snapshot
        );
    }
}
