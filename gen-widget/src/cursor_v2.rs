...
impl ViewportCursor {
    ...
    pub fn to_world_pos(&self, viewport_graph: &ViewportGraph) -> Option<WorldPos> {
    ...
    /// Alias for viewport_pos()
    pub fn get_viewport_pos(&self) -> ViewportPos {
        self.viewport_pos()
    }

    /// Alias for node_idx()
    pub fn get_node_idx(&self) -> Option<NodeIndex> {
        self.node_idx()
    }
    ...
    fn jump_to_adjacent_layer(
        ...
        let mut candidates: Vec<(WorldPos, &crate::layout::LayoutNode)> = viewport_graph
            .find_adjacent_data_nodes(node_idx)
            .into_iter()
            .filter_map(|(pos, node, _path)| {
                // Extract domain index from node role
        ...
        // Sort by vertical distance (prefer same Y level), then by -y (highest y wins)
...
mod tests {
    ...
    fn create_mock_viewport_graph() -> (ViewportGraph, Vec<(NodeIndex, WorldPos, (u64, u64))>) {
