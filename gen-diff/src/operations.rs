...
pub fn collect_operation_diff(
    ...
    let added_graphs = build_block_group_diffs(op_conn, &added_ops, db_path)?;
    let removed_graphs = build_block_group_diffs(op_conn, &removed_ops, db_path)?;
...
mod tests {
    ...
    fn one_operation_diff() {
        ...
        let diffs = collect_operation_diff(&op_conn, base_op.hash, head.hash, None).expect("diff");
    ...
    fn diff_against_itself_is_empty() {
        ...
        let base =
            Operation::create(&op_conn, "seed", &HashId::pad_str(1)).expect("create base op");
        let diffs = collect_operation_diff(&op_conn, base.hash, base.hash, None).expect("diff");
    ...
    fn filters_by_database_path() {
        ...
        let diffs = collect_operation_diff(&op_conn, base.hash, op_two.hash, Some("db-one.db"))
            .expect("diff");
        ...
        let diff_none =
            collect_operation_diff(&op_conn, base.hash, op_two.hash, Some("missing.db"))
                .expect("diff");
