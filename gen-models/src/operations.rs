...
use gen_core::{CommitHash, HashId, Workspace, calculate_hash};
...
fn operation_log_id(operation_kind: &str, summary: &str) -> HashId {
    let timestamp = chrono::Utc::now()
        .timestamp_nanos_opt()
        .expect("should create operation log timestamp");
    HashId(calculate_hash(&format!(
        "{operation_kind}:{summary}:{timestamp}"
    )))
}

fn asset_ref_id_for_record(asset: &OperationAssetRecord<'_>) -> HashId {
    HashId(calculate_hash(&format!(
        "{uri}:{file_type}:{checksum}:{role}:{logical_path}:{name}",
        uri = asset.file_addition.asset_uri,
        file_type = asset.file_addition.file_type.as_str(),
        checksum = asset.file_addition.checksum,
        role = asset.role,
        logical_path = asset.logical_path.unwrap_or_default(),
...
#[cfg_attr(
    feature = "profiling",
    tracing::instrument(skip(context, operation_info, summary))
)]
pub fn commit_graph_operation(
    context: &DbContext,
    operation_info: &OperationInfo,
    summary: &str,
) -> Result<CommitHash, OperationError> {
    ...
    let file_additions = operation_info
        .files
        .iter()
        .map(|operation_file| {
            FileAddition::prepare(
                workspace,
    ...
    let tracked_assets = file_additions
        .iter()
        .zip(operation_info.files.iter())
        .map(|(file_addition, operation_file)| OperationAssetRecord {
            file_addition,
            role: "input",
            logical_path: Some(operation_file.file_path.as_str()),
            name: Some(operation_file.filename.as_str()),
        })
        .collect::<Vec<_>>();
    track_operation_assets(
        graph_conn,
        &operation_log_id(&operation_info.description, summary),
        &operation_info.description,
        summary,
        &tracked_assets,
    )?;

    let commit_result = history_store.commit_all(summary);
...
pub(crate) fn track_operation_assets(
    ...
    let created_on = chrono::Utc::now().timestamp_nanos_opt().unwrap();
    let mut log_stmt = graph_conn.prepare_cached(
        "INSERT OR IGNORE INTO gen_operation_log (id, operation_kind, command, created_on) \
         VALUES (?1, ?2, ?3, ?4)",
    )?;
    log_stmt.execute(params![log_id, operation_kind, command, created_on])?;

    let mut asset_ref_stmt = graph_conn.prepare_cached(
        "INSERT OR IGNORE INTO gen_asset_refs \
         (id, uri, file_type, checksum, size, role, logical_path, name, created_on) \
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)",
    )?;
    let mut operation_asset_stmt = graph_conn.prepare_cached(
        "INSERT OR IGNORE INTO gen_operation_assets (log_id, asset_ref_id, role) \
         VALUES (?1, ?2, ?3)",
    )?;

    for asset in assets {
        let asset_ref_id = asset_ref_id_for_record(asset);
        asset_ref_stmt.execute(params![
            asset_ref_id,
            asset.file_addition.asset_uri,
            asset.file_addition.file_type.as_str(),
            asset.file_addition.checksum,
            Option::<i64>::None,
            asset.role,
            asset.logical_path,
            asset.name,
        ...
        operation_asset_stmt.execute(params![log_id, asset_ref_id, asset.role])?;
    }
...
pub fn add_files_operation(
    ...
    files: &[String],
    message: Option<&str>,
) -> Result<CommitHash, AddFilesOperationError> {
    let workspace = context.workspace();
    let graph_conn = context.graph().conn();
    ...
    let tracked_assets = unique_file_additions
        ...
        .collect::<Vec<_>>();
    track_operation_assets(graph_conn, &log_id, "add-file", &summary, &tracked_assets)?;
    let history_store = DoltHistoryStore::new(graph_conn);
...
impl Defaults {
    ...
    pub fn get(conn: &ConfigConnection) -> Option<Defaults> {
        let query = "SELECT id, collection_name, remote_name, current_branch_name FROM defaults WHERE id = 1";
...
mod tests {
    ...
    fn test_add_files_operation_stores_shared_unmodified_asset_once_across_operations() {
        ...
        let first_assets = OperationAsset::query(
            graph_conn,
            "SELECT * FROM gen_operation_assets WHERE log_id = ?1 ORDER BY asset_ref_id",
            params![operation_logs[0].id],
        );
        let second_assets = OperationAsset::query(
            graph_conn,
            "SELECT * FROM gen_operation_assets WHERE log_id = ?1 ORDER BY asset_ref_id",
            params![operation_logs[1].id],
        );
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut asset_refs =
            AssetRef::query(graph_conn, "SELECT * FROM gen_asset_refs ORDER BY name", []);
    ...
    fn test_add_files_operation_tracks_graph_asset_refs() {
        ...
        let asset_refs = AssetRef::query(
            graph_conn,
            "SELECT * FROM gen_asset_refs ORDER BY logical_path",
            [],
        );
        let operation_logs = OperationLog::query(
            graph_conn,
