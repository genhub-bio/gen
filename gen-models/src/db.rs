...
mod tests {
    ...
    use super::{ConfigConnection, DbContext, get_connection};
    use crate::{
        collection::Collection,
        history::dolt::{active_branch, commit_all, connect_branch, create_branch},
        migrations::run_operation_migrations,
        operations::Defaults,
        sample::{NewSample, Sample},
    };

    fn open_config_connection(path: &std::path::Path) -> ConfigConnection {
        let mut conn = Connection::open(path).expect("should open config database");
    ...
    fn setup_branch_restore_repo() -> (Workspace, std::path::PathBuf, std::path::PathBuf) {
        ...
        let config_conn = open_config_connection(&config_path);
        ...
        (workspace, graph_path, config_path)
    }

    #[test]
    fn test_new_restores_saved_current_branch() {
        let (workspace, graph_path, config_path) = setup_branch_restore_repo();
