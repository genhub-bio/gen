...
/// the raw material `expand` merges into a graph. Fetching the neighbors' own edges too (not
/// just `node_id`'s) gives every neighbor its complete set of slice boundaries.
